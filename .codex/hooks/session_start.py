...
def main() -> None:
    ...
    lines = [
        ...
        f"- git tracking: {'project-local' if has_project_local_git(repo_root) else 'fallback (no project-local git)'}",
        f"- mode: {mode}",
    ...
    if not has_project_local_git(repo_root):
        lines.append("- note: Stop hook uses project-local mtime fallback because this directory is not its own git repo.")
